...
import argparse  #argparse命令行参数解析包
import sys
import numpy as np
import cv2
import os
from win32.win32api import GetSystemMetrics
#GetSystemMetrics只有一个参数，称之为「索引」，这个索引有75个标识符，可以获取系统分辨率、窗体显示区域的宽度和高度、滚动条的宽度和高度。
...
def drawBitmap(w_image):
    ...
    # 定义 迭代终止准则, 需要的聚类的数量(K) 和使用KMeans聚类函数()
    ...
    for i in center:
        ...
        os.system('potrace.exe .tmp.bmp -s --flat')#使用potrace工具将位图转换为矢量图形
...
if __name__ == '__main__':
    paser = argparse.ArgumentParser(
        description="将位图转换为SVG并使用turtle libray来绘制它。")
    paser.add_argument('filename', type=str,
                       help='该文件(*.jpg, *.png, *.bmp) 要转换的文件的名称。')
    paser.add_argument(
        "-c", "--color", help="你想要画多少种颜色。(如果数字太大，程序可能会很慢。)有32、64种", type=int, default=32)
    args = paser.parse_args()
    K = args.color
    try:
        bitmapFile = open(args.filename, mode='r')
    except FileNotFoundError:
        print(__file__ + ': 错误:文件不存在。')
        quit()

    if os.path.splitext(args.filename)[1].lower() not in ['.jpg', '.bmp', '.png']:
        print(__file__ + ': 错误:该文件不是位图文件。')
        quit()
...
```
python draw_anything.py [xxxx].jpg即可运行